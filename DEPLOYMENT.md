...
| [PlanetScale](https://planetscale.com) | The app's MySQL database | 5GB storage, generous row-read limits |
...
Render has no managed MySQL (Postgres only), and self-hosting Ollama for Task 2's LLM step needs an always-on ~$25/mo instance for enough RAM — not free. PlanetScale + Neon + Groq fill exactly those two gaps at $0/mo.

---

## 1. PlanetScale — the app's database

1. Sign up at planetscale.com, create a new **MySQL** database (not Postgres — this app uses `PyMySQL` and MySQL-specific SQL).
2. From the database's **Connect** panel, copy the host, username, password, and database name (Render/general connection tab, not a specific ORM's tab).
3. **Known risk to check**: this schema uses `FOREIGN KEY` constraints (`db/schema.sql`). Most current PlanetScale plans support them, but if the app's first startup fails with a foreign-key-related SQL error, the fix is safe and mechanical: open `db/schema.sql`, delete the 4 lines reading `FOREIGN KEY (person_id) REFERENCES persons(person_id)` (one each in `applicant_details`, `gig_worker_details`, `cbnexus_contacts`, `audio_submissions`), commit, redeploy. Nothing in the app relies on FK-enforced behavior (no cascading deletes anywhere) — this is purely a compatibility fix, not a functional change.
...
3. Before clicking **Apply**, Render will prompt for every `sync: false` env var. Fill them in:
   ...
   **nexora-app** and **nexora-api** (same MySQL values for both, from PlanetScale step 1):
   - `MYSQL_HOST`, `MYSQL_USER`, `MYSQL_PASSWORD`, `MYSQL_DATABASE` — from PlanetScale
...
- **nexora-app**: visit its `.onrender.com` URL. First load triggers automatic schema creation on PlanetScale (see step 1's FK note if this fails) and seeds your admin account from `ADMIN_USERNAME`/`ADMIN_PASSWORD`. Log in, confirm the Data Merge Engine page loads.
...
5. Run `pipeline/merge.py` once against the PlanetScale DB (or use the Data Merge Engine page) so there's actual data to tag, then click **Execute Workflow**.
...
- **PlanetScale FK support** — see step 1.
